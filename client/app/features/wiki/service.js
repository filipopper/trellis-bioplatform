...
export class WikiService {
  ...
  buildArticleResourceUrls(slug) {
    const articleUrl = resolveWikiResourceUrl(slug, 'article.json');
    const contentUrl = resolveWikiResourceUrl(slug, 'content.md');
  ...
  async loadArticle(slug) {
    ...
    const { articleUrl, contentUrl } = this.buildArticleResourceUrls(normalizedSlug);
    if (!articleUrl || !contentUrl) {
      reportWarning('wiki.service.loadArticle', 'Wiki resource URL resolution failed', { slug: normalizedSlug, articleUrl, contentUrl });
      return { kind: 'error', slug: normalizedSlug, meta };
    }

    try {
      const [articleRes, contentRes] = await Promise.all([fetch(articleUrl), fetch(contentUrl)]);

      if (!articleRes.ok || !contentRes.ok) {
        reportWarning('wiki.service.loadArticle', 'Article resource returned non-OK status', {
          slug: normalizedSlug,
      ...
      try {
        ...
        reportError('wiki.service.loadArticle.parseArticleJson', error, { slug: normalizedSlug, articleUrl });
        return { kind: 'error', slug: normalizedSlug, meta };
      }
      ...
      try {
        ...
        reportError('wiki.service.loadArticle.readContentMarkdown', error, { slug: normalizedSlug, contentUrl });
        return { kind: 'error', slug: normalizedSlug, meta };
      }
      ...
      reportError('wiki.service.loadArticle.fetchResources', error, { slug: normalizedSlug, articleUrl, contentUrl });
      return { kind: 'error', slug: normalizedSlug, meta };
    }
  }
}
